...
import { DemeritsDashboard } from "@/components/DemeritsDashboard";
import { supabase } from "@/integrations/supabase/client";

export const Route = createFileRoute("/demerits")({
  head: () => ({
    meta: [
      { title: "Deméritos — Vamo Dale!!" },
      { name: "description", content: "Registre comportamentos que você quer reduzir e desconte XP quando eles acontecerem." },
    ],
  }),
  component: DemeritsPage,
});

function DemeritsPage() {
  ...
  return (
    <main className="min-h-screen bg-[#f7f8fb] text-slate-950">
      <header className="sticky top-0 z-30 border-b border-slate-200 bg-white/95 backdrop-blur">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-5 py-4 lg:px-8">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-sm font-medium text-slate-500">Vamo Dale!!</p><h1 className="flex items-center gap-2 font-bold"><ShieldAlert className="size-4 text-red-600" /> Deméritos</h1></div>
          </div>
      ...
      <div className="mx-auto max-w-7xl px-5 py-7 lg:px-8"><DemeritsDashboard userId={userId} /></div>
    </main>
  );
}
