      ...
      {showForm && (
        <form onSubmit={submit} className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-7">
          ...
          <div className="grid gap-5 md:grid-cols-2">
            ...
            <label><span className="mb-2 block text-sm font-bold text-slate-700">XP por check-in</span><input type="number" min="0" step="1" className="input" value={form.xpReward} onChange={(e) => setForm({ ...form, xpReward: Math.max(0, Math.floor(Number(e.target.value) || 0)) })} /></label>
          </div>
